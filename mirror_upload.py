...
config_folder = "mirror_upload/"
config_creds = "mirror_upload/credentials.json"
config_devices = "mirror_upload/devices/"
...
def create_creds():
    ...
    pseudo = input(TRESET + "Enter your SSH username: ")
    password = input(TRESET + "Enter your SSH password: ")
    android_version = input(TRESET + "Enter the EvoX version you are uploading (Ex: 9, 9-test, etc): ")
    os.environ["REMOTE_USERNAME"] = pseudo

    with open(config_creds, "w") as f:
        f.write(f'{{"username": "{pseudo}", "password": "{password}", "EvolutionX Version": "{android_version}"}}')
...
def upload_rom_file(device):
    rom_file = out + device + "/EvolutionX*.zip"
    if not os.path.exists(rom_file):
        print("ROM file not found. Exiting...")
        return
    else:
        print("ROM file found. Uploading...")
        os.system(f"rsync -avz --progress --partial --inplace --rsh=ssh {rom_file} {remote_path}")
    ...
    if re.match(pattern_rom, filename):
        try:
            if not os.path.exists(file_path):
                ...
                sending_path = f"{remote_path}/{android_version}/{device}/"
                ssh.connect(hostname=remote_host, username=username, password=password)
                scp = SCPClient(ssh.get_transport())
                print(TYELLOW + f"Sending {file_path} to {sending_path}... Please wait...")
                scp.put(file_path, f"{sending_path}")
                scp.close()
                print(TGREEN + f"Upload successful !")
        ...
        try:
            if not os.path.exists(file_path):
                ...
                sending_path = f"{remote_path}/{device}/{android_version}/{filename_without_ext}/"
                ssh.connect(hostname=remote_host, username=username, password=password)
                scp = SCPClient(ssh.get_transport())
                print(TYELLOW + f"Sending {file_path} to {sending_path}... Please wait...")
                scp.put(file_path, f"{sending_path}")
